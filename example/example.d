...
int main(string[] args)
{
	...
	enforce((ret=sd_journal_open(&journal,0))==0), // open all journal file types
		new Exception("Problem opening journal: error "~to!string(ret));
	scope(exit)
		sd_journal_close(journal);

	string match="_COMM=sshd";
	enforce((ret=sd_journal_add_match(journal,toZString(match),match.length)==0),
	 	new Exception("Problem opening journal: error "~to!string(ret)));
	enforce((ret=sd_journal_seek_head(journal)==0),
	 	new Exception("Problem opening journal: error "~to!string(ret)));
	sd_journal_next(journal);
	ulong size=buf.sizeof;
	cp=cast(void*)buf.ptr;
	//enforce((ret=sd_journal_get_data(journal,toZString("MESSAGE="),cast(void**)&cp,&size))==0),
	 //	new Exception("Problem opening journal: error "~to!string(ret));
	//writefln("MESSAGE=%s",ZtoString(cast(const char*)buf));
         	size_t l=buf.sizeof;
         	for (sd_journal_restart_data(journal); (sd_journal_enumerate_data(journal, cast(void**)&cp,&(l)) > 0); )
         	{
         		writefln("%s:%s",l,ZtoString(cast(const char*)((cast(char*)(cp))[0..l]~'\0')));
         		l=buf.sizeof;
         		cp=cast(void*)buf.ptr;
         	}
         	return 0;
